...
class Mutation_Handler():
    ...
    def remove_genotype_parts(self, new_size, dna_dict):
        while dna_dict['size'] > new_size and dna_dict['size'] > 1:
            index_to_remove = ng.generate_n_uniform_random_integers(min_val = 0 , max_val = dna_dict['size'] , size = 1 )[0]
            for key in dna_dict:
                if key == 'size':
                    continue
    ...
    def add_genotype_parts(self, new_size, dna_dict):
        while dna_dict['size'] < new_size:
            index_to_add = ng.generate_n_uniform_random_integers(min_val = 0 , max_val = dna_dict['size'] , size = 1 )[0]
            self.insert_new_values_to_dna(index_to_add, dna_dict)
            

    def insert_new_values_to_dna(self, index, dna_dict):
        dna_dict['feature_indices'].insert(index , ng.generate_n_uniform_random_integers(min_val=0, max_val = constants.MAX_NUM_OF_FEATURES, size = 1)[0])
        dna_dict['weights'].insert(index, ng.generate_n_uniform_random_values(1)[0])
        dna_dict['parentheses_binary_vec'].insert(index, ng.generate_n_uniform_random_integers(min_val=0 , max_val=2 , size=1)[0])
        dna_dict['actions'].insert(index, ng.generate_n_uniform_random_integers(min_val = 0 , max_val = constants.NUM_ACTIONS, size = 1)[0])
        dna_dict['size'] += 1
